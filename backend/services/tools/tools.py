...
async def execute_shell(command: str, websocket=None) -> str:
    ...
    if not is_safe:
        ...
        return f"BLOCKED: {reason}. This command is not allowed for security reasons."
            ...
            if "ModuleNotFoundError: No module named" in err_msg:
                try:
                    ...
                    if match:
                        ...
                        if websocket:
                            import json
                            await websocket.send_text(json.dumps({"role": "terminal-output", "content": f"\n[Auto-Fix] Installing missing module: {module_name}...\n"}))
